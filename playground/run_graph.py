...
class GraphRunner:
    ...
    def stop(self):
        ...
        if self.interpreter and self.interpreter.is_executing:
            try:
                self.interpreter.stop_execution()
            except Exception as e:
                print(f"   ⚠️  Error stopping execution: {e}")
    ...
    def _print_stats(self):
        ...
        elapsed = time.time() - self._start_time
        stats = self.interpreter.get_statistics() if self.interpreter else {}

        print(
            f"   [{elapsed:6.1f}s] "
            f"Subscriptions: {stats.get('total_subscriptions', 0)} | "
            f"Schedulers: {len(stats.get('schedulers', []))}"
        )
...
Examples:
  ...
  %(prog)s saves/my_graph.json -v
    ...
    parser.add_argument("graph_file", type=str, help="Path to the graph JSON file")

    parser.add_argument("--fps", "-f", type=float, default=60.0, help="Target framerate (default: 60)")
    ...
    parser.add_argument("--verbose", "-v", action="store_true", help="Print periodic statistics")
