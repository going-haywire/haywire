...
class SyncGraphRunner:
    ...
    def _assemble_flows(self):
        ...
        if self.flow_type in self.flows:
            ...
            raise RuntimeError(
                f"Flow '{self.flow_type}' not found. Available: {', '.join(available)}"
            )
    ...
    def _get_friendly_name(self, subscription_key: str) -> str:
        ...
        name_map = {
            SystemEvent(SystemEventType.TICK).get_subscription_key(): 'tick',
    ...
    def list_flows(self) -> List[str]:
        ...
        return [self._get_friendly_name(f.get_subscription_key()) for f in self.flows.values() 
...
Examples:
  ...
  %(prog)s saves/graph.json --flow tick --no-startup
    ...
    parser.add_argument(
        '--no-startup',
        action='store_true',
        help='Skip running BEGIN_PLAY before main flow'
    )
