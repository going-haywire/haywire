...
class MockNodeBase:
    ...
    def _analyze_worker_signature(self) -> None:
        ...
        context_param = params.pop('context', None)
...
class TestWorkerSignaturePerformance:
    ...
    def test_results_match(self):
        ...
        for op in MathOP:
            ...
            assert abs(legacy_result - new_result) < 0.001, \
                f"Results differ for operation {op.value}"
    ...
    def test_performance_comparison(self):
        ...
        # Report results
        print(f"\n{'='*60}")
        print(f"Worker Signature Performance Comparison")
        print(f"{'='*60}")
        print(f"Iterations: {iterations:,}")
        print(f"")
        print(f"Legacy approach (self.value()):")
        print(f"  Total time:    {legacy_time_ns / 1_000_000:.2f} ms")
        print(f"  Per-call:      {legacy_per_call_ns:.1f} ns")
        print(f"")
        print(f"New approach (cached kwargs):")
    ...
    def test_performance_with_varying_port_counts(self):
        ...
        for port_count in [1, 3, 5, 10]:
            ...
            legacy = self._create_dynamic_node(
                port_count, 
                use_new_signature=False
            )
            
            # Create dynamic new signature node
            new = self._create_dynamic_node(
                port_count, 
        ...
        # Report
        print(f"\n{'='*60}")
        print(f"Performance Scaling with Port Count")
        print(f"{'='*60}")
    ...
    def _create_dynamic_node(
        self, 
        port_count: int, 
        use_new_signature: bool
    ) -> MockNodeWrapper:
        """
...
class DynamicNewNode(MockNodeBase):
    ...
    def initialize(self):
        ...
        self._worker_port_refs = [
            (f'input_{i}', self.ports[f'input_{i}'])
            for i in range(self.port_count)
        ]
        self._uses_legacy_worker = False
...
def run_detailed_analysis():
    ...
    print(f"Legacy (3x self.value):   {legacy_extract/1_000_000:.2f} ms ({legacy_extract/iterations:.1f} ns/call)")
    print(f"New (_extract_kwargs):    {new_extract/1_000_000:.2f} ms ({new_extract/iterations:.1f} ns/call)")
    ...
    print(f"Reuse dict (mutate):      {reuse_dict/1_000_000:.2f} ms ({reuse_dict/iterations:.1f} ns/call)")
    ...
    print(f"Direct positional call:   {direct_call/1_000_000:.2f} ms ({direct_call/iterations:.1f} ns/call)")
    print(f"**kwargs unpacking:       {unpack_call/1_000_000:.2f} ms ({unpack_call/iterations:.1f} ns/call)")
    print(f"Pure **kwargs function:   {kwargs_call/1_000_000:.2f} ms ({kwargs_call/iterations:.1f} ns/call)")
    ...
    print(f"self.value('port_id'):    {method_call/1_000_000:.2f} ms ({method_call/iterations:.1f} ns/call)")
    print(f"Cached closure call:      {closure_call/1_000_000:.2f} ms ({closure_call/iterations:.1f} ns/call)")
    print(f"Direct port.get_value():  {direct_attr/1_000_000:.2f} ms ({direct_attr/iterations:.1f} ns/call)")
    ...
    print(f"Hardcoded tuple:          {tuple_hardcoded/1_000_000:.2f} ms ({tuple_hardcoded/iterations:.1f} ns/call)")
    print(f"Generator tuple:          {tuple_generator/1_000_000:.2f} ms ({tuple_generator/iterations:.1f} ns/call)")
    print(f"List comp → tuple:        {tuple_listcomp/1_000_000:.2f} ms ({tuple_listcomp/iterations:.1f} ns/call)")
    print(f"List direct (no tuple):   {list_direct/1_000_000:.2f} ms ({list_direct/iterations:.1f} ns/call)")
    print(f"")
    ...
    print(f"Generator vs Legacy:      {legacy_extract/tuple_generator:.2f}x {'faster' if tuple_generator < legacy_extract else 'slower'}")
    ...
    def make_specialized_extractor(p_a, p_b, p_op):
        def extract():
            return {
                'value_a': p_a.get_value(),
                'value_b': p_b.get_value(), 
                'operator': p_op.get_value()
            }
        return extract
    ...
    print(f"Specialized closure:      {specialized_time/1_000_000:.2f} ms ({specialized_time/iterations:.1f} ns/call)")
    ...
    print(f"Reusable dict closure:    {reusable_time/1_000_000:.2f} ms ({reusable_time/iterations:.1f} ns/call)")
    ...
    print(f"Direct port injection:    {direct_inject/1_000_000:.2f} ms ({direct_inject/iterations:.1f} ns/call)")
    ...
    print(f"Cached names extraction:  {getattr_time/1_000_000:.2f} ms ({getattr_time/iterations:.1f} ns/call)")
    ...
    for port_count in [2, 3, 5, 6, 10, 20]:
        ...
        if port_count == 1:
            ...
            extract_hybrid = make_tuple_extractor_5(ports[0], ports[1], ports[2], 
                                                     ports[3], ports[4])
            hybrid_type = "tuple"
        else:
        ...
        print(f"  Legacy:         {time_legacy/iters:>6.1f} ns  ({speedup_vs_legacy:.2f}x slower than hybrid)")
