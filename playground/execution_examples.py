...
def _create_graph_with_math(graph: BaseGraph) -> BaseGraph:
    ...
    begin_play = graph.create_node_wrapper(BeginPlayNode.class_identity.registry_key, position=(100, 100))

    print_msg = graph.create_node_wrapper(LoggerNode.class_identity.registry_key, position=(300, 100))
...
def example_simple_flow():
    ...
    # Create graph
    graph = BaseGraph(graph_id="simple_example", name="Simple Flow Example")
...
def example_sequence_flow():
    ...
    # Create graph
    graph = BaseGraph(graph_id="sequence_example", name="Sequence Flow Example")

    # Create nodes
    begin_play = graph.create_node_wrapper(BeginPlayNode.class_identity.registry_key, position=(100, 100))

    print_msg1 = graph.create_node_wrapper(PrintMessageNode.class_identity.registry_key, position=(300, 100))

    print_msg2 = graph.create_node_wrapper(PrintMessageNode.class_identity.registry_key, position=(500, 100))

    # Connect control flow: BeginPlay → PrintMessage1 → PrintMessage2
    graph.create_edge_wrapper(begin_play.node_id, "exec", print_msg1.node_id, "exec")

    graph.create_edge_wrapper(print_msg1.node_id, "done", print_msg2.node_id, "exec")
...
def example_callback_string_based():
    ...
    # Create graph
    graph = BaseGraph(graph_id="callback_string_example", name="Callback String-Based Example")
    ...
    emit_callback.node.ports["custom_name"].set_value(True)  # Enable custom name
    ...
    callback_listener.node.ports["custom_name"].set_value(True)  # Enable custom name
    callback_listener.node.ports["custom_callback_name"].set_value("my_callback")

    print_msg = graph.create_node_wrapper(PrintMessageNode.class_identity.registry_key, position=(300, 300))

    graph.create_edge_wrapper(callback_listener.node_id, "triggered", print_msg.node_id, "exec")
...
def example_callback_edge_based():
    ...
    # Create graph
    graph = BaseGraph(graph_id="callback_edge_example", name="Callback Edge-Based Example")
    ...
    # Leave custom_name=False (default) for edge-based mode
    ...
    callback_edge = graph.create_edge_wrapper(
        callback_listener.node_id,
        "listen_callback",  # Source: listener's outlet
        emit_callback.node_id,
        "edge_callback",  # Target: emitter's inlet
    )
...
def example_external_events():
    ...
    # Create graph
    graph = BaseGraph(graph_id="external_event_example", name="External Event Example")

    # Create key press listener
    # (This would be a KeyPressedNode in real implementation)
    begin_play = graph.create_node_wrapper(BeginPlayNode.class_identity.registry_key, position=(100, 100))

    print_msg = graph.create_node_wrapper(PrintMessageNode.class_identity.registry_key, position=(300, 100))
    ...
    interpreter.dispatch_external_event(
        category="input", name="key_pressed", binding_id={"key": "Space", "modifiers": []}
    )

    # Dispatch network event
    interpreter.dispatch_external_event(
        category="network", name="message_received", binding_id={"from": "server", "data": "Hello!"}
    )
