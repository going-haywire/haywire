...
- Grounding facts already established while charting (verify, don't re-derive):
  - Studio entry: `HaywireApp` in `packages/haywire-studio/src/haywire_studio/app.py`, runs `ui.run(port=8082)` on NiceGUI/FastAPI; shared services + `LibraryRegistry` are process-global, sessions are per browser client via `SessionManager`.
