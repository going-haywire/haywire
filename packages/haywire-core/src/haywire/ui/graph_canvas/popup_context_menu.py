...
class PopupContextMenu:
    ...
    def _setup_hot_reload_listener(self):
        ...
        self.node_factory.add_batch_listener(lambda event: self._menu_builder.invalidate_cache())
    ...
    def _create_node(self, registry_key: str):
        ...
        event = NodeCreateRequestEvent(registryKey=registry_key, position={"x": canvas_x, "y": canvas_y})
    ...
    def _paste_clipboard(self):
        ...
        event = UserPasteClipboardEvent(canvasX=canvas_x, canvasY=canvas_y)
        self._on_emit_event(event)
        self._close_current_menu()
    ...
    def _copy_node(self, node_id: str):
        ...
        event = UserCopySelectedEvent(selectedNodes=[node_id], selectedEdges=[])
        self._on_emit_event(event)
        self._close_current_menu()
    ...
    def show_node_menu(self, x: float, y: float, node_id: str):
        ...
        with popup:
            with ui.column().classes("w-full gap-1"):
                ...
                btn3.classes(
                    "w-full justify-start px-3 py-2 text-red-600 hover:bg-red-50 hover:text-red-700 text-sm"
                )
    ...
    def show_edge_menu(self, x: float, y: float, edge_id: str, edge: Edge, state: EdgeWrapperState):
        """Show context menu for connection operations with detailed metrics."""
        self._close_current_menu()

        # Store edge data for operations
        self._menu_data = {"edge_id": edge_id, "edge": edge, "state": state, "x": x, "y": y}
        ...
        with popup:
            with ui.column().classes("w-full gap-1"):
                ...
                ui.label(f"Status: {status_icon}").classes(f"text-sm {status_color} px-3 py-1 font-bold")
                ...
                btn2 = ui.button("🗑️ Delete Connection", on_click=lambda: self._delete_connection(edge_id))
                btn2.props("flat align=left")
                btn2.classes(
                    "w-full justify-start px-3 py-2 text-red-600 hover:bg-red-50 hover:text-red-700 text-sm"
                )
    ...
    def show_selected_menu(
        self, x: float, y: float, selected_nodes: List[str], selected_connections: List[str]
    ):
        """Show context menu for multi-selection operations."""
        self._close_current_menu()

        # Store selection data for operations
        self._menu_data = {"selected_nodes": selected_nodes, "selected_connections": selected_connections}
        ...
        if node_count > 0 and connection_count > 0:
            ...
            title = (
                f"Selection ({connection_count} {'connection' if connection_count == 1 else 'connections'})"
            )
        else:
            title = "Selection"
    ...
    def _delete_selected(self):
        ...
        event = UserRemoveEvent(nodes=selected_nodes, edges=selected_connections)
        self._on_emit_event(event)
    ...
    def _copy_selected_nodes(self):
        ...
        event = UserCopySelectedEvent(selectedNodes=selected_nodes, selectedEdges=selected_connections)
        self._on_emit_event(event)
        self._close_current_menu()
