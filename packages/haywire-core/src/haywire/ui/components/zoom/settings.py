...
class EditorPanZoomSettings(FrameworkSettings, namespace=NAMESPACE_EDITOR_PAN_ZOOM):
    ...
    cull_enabled = setting[BOOL](
        ...
        description=(
            "Stop rendering nodes that are off screen. Improves UX performance on large graphs, "
            "but may cause jerky pan and zoom behaviour."
        ),
        category=CATEGORY_EDITOR_CULLING,
    )

    cull_min_nodes = setting[INT](
        100,
        label="Node Threshold",
        description=("Start culling with more than this many nodes"),
        category=CATEGORY_EDITOR_CULLING,
        min=0,
        max=5000,
    )
    cull_min_zoom = setting[FLOAT](
        0.2,
        label="Zoom Threshold",
        description=("Only cull at or above this zoom level."),
        category=CATEGORY_EDITOR_CULLING,
        min=0.0,
        max=5.0,
    )
