...
class FlowAssemblyManager:
    ...
    def assemble_graph(self, graph: "BaseGraph") -> List[Flow]:
        ...
        for event_node in event_nodes:
            try:
                ...
                logger.error(
                    f"Failed to assemble flow from event node {event_node.node_id}: {e}", exc_info=True
                )
                raise
    ...
    def _assemble_flow(self, event_node: "BaseNode", graph: "BaseGraph") -> Flow:
        """
        ...
        for node_id, node_info in control_graph.control_nodes.items():
            try:
                localized_flow = DataFlowBuilder.build_localized(node_info.node, control_graph, graph)
                if len(localized_flow.execution_sequence) > 0:
                    node_info.localized_data_flow = localized_flow

            except Exception as e:
                logger.error(f"Failed to build data flow for {node_id}: {e}", exc_info=True)
                raise
    ...
    def _process_callback_edges(self, graph: "BaseGraph", flows: List[Flow]):
        """
        ...
        for edge in callback_edges:
            ...
            if source_wrapper and target_wrapper:
                ...
                if source_port and target_port:
                    ...
                    logger.debug(f"    Event propagation: '{source_event}' → {target_wrapper.node_id}")
        ...
        # Log summary
        logger.info(f"Callback topology: {len(callback_edges)} emitters, {len(callback_triggers)} listeners")
            ...
            "flows": [
                {
                    ...
                    "emits_callbacks": (flow.entry_event_node.node_id in callback_edges),
