...
class DataFlowBuilder:
    ...
    @staticmethod
    def build_localized(
        control_node: "BaseNode", control_graph: "ControlFlowGraph", graph: "BaseGraph"
    ) -> "LocalizedDataFlow":
        """
        ...
        logger.debug(f"Building localized data flow for {control_node.node_id}")
        ...
        data_inlets = [
            port
            for port in control_node.ports.values()
        ...
        for inlet in data_inlets:
            ...
            dependencies = DataFlowBuilder._backpropagate(
                control_node.node_id, inlet.id, graph, control_graph
            )
        ...
        DataFlowBuilder._check_cycles(required_nodes, graph, control_graph)

        # Topological sort for execution order
        execution_sequence = DataFlowBuilder._topological_sort(required_nodes, graph)
    ...
    @staticmethod
    def _backpropagate(
        start_node_id: str, inlet_id: str, graph: "BaseGraph", control_graph: "ControlFlowGraph"
    ) -> Set[str]:
        """
        ...
        while queue:
            ...
            for edge in edge_wrappers:
                ...
                if source_wrapper:
                    ...
                    is_control = any(
                        port.flow_type == FlowType.CONTROL for port in source_node.ports.values()
                    )
    ...
    @staticmethod
    def _check_cycles(node_ids: Set[str], graph: "BaseGraph", control_graph: "ControlFlowGraph"):
        """
        ...
        for node_id in node_ids:
            ...
            outlets = [
                port for port in node.ports.values() if port.flow_type == FlowType.DATA and port.is_outlet()
            ]
    ...
    @staticmethod
    def _topological_sort(node_ids: Set[str], graph: "BaseGraph") -> List[str]:
        """
