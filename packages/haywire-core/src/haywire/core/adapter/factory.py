...
class AdapterFactory:
    ...
    def __init__(self, adapter_registry: AdapterRegistry):
        ...
        self.adapter_registry.add_batch_event_subscriber(self._on_adapter_lifecycle_event)

    def create_chain(
        self, source_type: type[IType], sink_type: type[IType], edge_id: str, max_depth: int = 3
        ...
        if not source_is_compound and not sink_is_compound:
            ...
            if source_type == sink_type:
                ...
                first_adapter, error = self._create_scalar_chain(source_type, sink_type, max_depth)
        ...
        if first_adapter and error is None:
            ...
            self._register_edge_dependencies(edge_id, first_adapter._get_registry_keys())
    ...
    def _create_scalar_chain(
        self, source_type: type[IType], sink_type: type[IType], max_depth: int
    ) -> tuple[Optional[IAdapter], Optional[str]]:
        """
        ...
        adapter_classes = self._find_chain_with_ancestors(source_type, sink_key, max_depth)

        if adapter_classes is None:
            return (None, (f"No adapter chain found: {source_type.__name__} → {sink_type.__name__}"))
        ...
        try:
            ...
            return (None, f"Failed to instantiate adapter chain: {e}")
    ...
    def _create_element_chain(
        ...
        if source_element is None or sink_element is None:
            return (None, "Compound types missing element_type_cls")
        ...
        if container_classes is None:
            return (None, (f"No container adapter found: {source_type.__name__} → {sink_type.__name__}"))

        # safety check - should only be one container adapter
        if len(container_classes) != 1:
            return (
                None,
                (f"Container transformation must be direct: found {len(container_classes)} adapters"),
            )
        ...
        if source_element == sink_element:
            try:
                ...
                return (None, f"Failed to instantiate container adapter: {e}")

        # Different element types - find element chain
        element_adapter, error = self._create_scalar_chain(source_element, sink_element, max_depth)
        ...
        try:
            ...
            return (None, f"Failed to inject element adapter: {e}")

    def _create_structural_chain(
        self, source_type: type[CompoundType], sink_type: type[CompoundType], max_depth: int
    ) -> tuple[Optional[IAdapter], Optional[str]]:
        """
        Case 3: Structural transformation.
        ...
        if structural_classes is None:
            return (None, (f"No structural adapter found: {source_type.__name__} → {sink_type.__name__}"))

        if len(structural_classes) != 1:
            return (
                None,
                (f"Structural transformation must be direct: found {len(structural_classes)} adapters"),
            )
        ...
        try:
            ...
            return (None, f"Failed to create structural chain: {e}")

    def register_edge_callback(self, edge_id: str, callback: LifeCycleBatchCallback):
    ...
    def _register_edge_dependencies(self, edge_id: str, adapter_keys: List[str]):
