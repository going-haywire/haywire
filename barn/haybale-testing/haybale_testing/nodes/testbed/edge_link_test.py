...
class EdgeLinkTestNode(BaseNode):
    ...
    def init(self):
        ...
        with self.rejig():
            ...
            self.add(TEST_BOOL.as_inlet(
                    id='bool_inlet',
                    label='Boolean Inlet',
                    default=True
                ))

            self.add(TEST_INT.as_inlet(
                    id='int_inlet',
                    label='Int Inlet',
                    default=42
                ))

            self.add(TEST_FLOAT.as_inlet(
                    id='float_inlet',
                    label='Float Inlet',
                    default=50.0
                ))

            self.add(TEST_STRING.as_inlet(
                    id='string_inlet',
                    label='String Input',
            ...
            self.add(TEST_TEMPERATURE.as_inlet(
                    id='temperature_inlet',
                    label='Temperature Inlet',
                    default=20.0
                ))

            self.add(PooledType[TEST_BOOL].as_inlet(
                    id='pooled_bool_inlet',
                    label='Pooled BOOL Inlet'
                ))

            self.add(PooledType[TEST_INT].as_inlet(
                    id='pooled_int_inlet',
                    label='Pooled INT Inlet'
                ))

            self.add(PooledType[TEST_FLOAT].as_inlet(
                    id='pooled_float_inlet',
                    label='Pooled FLOAT Inlet'
                ))

            self.add(PooledType[TEST_TEMPERATURE].as_inlet(
                    id='pooled_temperature_inlet',
                    label='Pooled Temperature Inlet'
                ))

            self.add(PooledType[TEST_STRING].as_inlet(
                    id='pooled_string_inlet',
                    label='Pooled STRING Inlet'
                ))

            self.add(PooledType[ArrayType[TEST_STRING]].as_inlet(
                    id='pooled_array_string_inlet',
                    label='Pooled ARRAY[STRING] Inlet'
                ))
            ...
            self.add(TEST_BOOL.as_outlet(
                    id='bool_outlet',
                    label='Boolean Output'
                ))

            self.add(TEST_INT.as_outlet(
                    id='int_outlet',
                    label='Integer Output'
                ))

            self.add(TEST_FLOAT.as_outlet(
                    id='float_outlet',
                    label='Float Output'
                ))

            self.add(TEST_TEMPERATURE.as_outlet(
                    id='temperature_outlet',
                    label='Temperature Output'
                ))

            self.add(TEST_STRING.as_outlet(
                    id='string_outlet',
                    label='Text Output'
                ))

            self.add(ArrayType[TEST_BOOL].as_outlet(
                    id='array_bool_outlet',
                    label='ARRAY[TEST_BOOL]'
                ))

            self.add(ArrayType[TEST_INT].as_outlet(
                    id='array_int_outlet',
                    label='ARRAY[TEST_INT]'
                ))

            self.add(ArrayType[TEST_FLOAT].as_outlet(
                    id='array_float_outlet',
                    label='ARRAY[TEST_FLOAT]'
                ))

            self.add(ArrayType[TEST_TEMPERATURE].as_outlet(
                    id='array_temperature_outlet',
                    label='ARRAY[TEST_TEMPERATURE]'
                ))

            self.add(ArrayType[TEST_STRING].as_outlet(
                    id='array_string_outlet',
                    label='ARRAY[TEST_STRING]'
                ))
