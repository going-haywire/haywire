...
class CanvasSettings(GlobalSettings):
    ...
    bg_pattern: str = setting('dots', label='Background Pattern', description='Pattern style for canvas background', category='ui.canvas', order=10, choices=['none', 'dots', 'lines', 'cross'])
    ...
    grid_subdivisions: int  = setting(5,    label='Grid Subdivisions', description='Minor grid lines per major line', category='ui.canvas', order=22, min=1,  max=10)
    snap_to_grid:      bool = setting(True, label='Snap to Grid',      description='Snap nodes to grid when moving', category='ui.canvas', order=23)
    ...
    zoom_to_cursor: bool  = setting(True, label='Zoom to Cursor', description='Zoom centred on cursor position',   category='ui.canvas', order=33)
