...
class MarketplaceState(AppState):
    ...
    async def fetch_overview(self, pkg: Haybale, *, cache_dir: "Path | None" = None) -> "str | None":
        ...
        Priority:
        1. ``notes`` — a bare filename inside the package directory, resolved
           against ``origin`` at ``install_spec``'s ref. Named by the author, so
           there is nothing to guess: the previous shape carried a *directory*
           and tried OVERVIEW.md then QUICKREF.md, which meant a page called
           anything else was unreachable and every miss cost a round trip.
        2. The module directory's OVERVIEW.md — derived from ``install_spec``,
           for a library that declares no ``notes``. Same convention the
           generated docs already follow.
        3. PyPI long_description fallback — only when the above yielded nothing
           and ``source == 'pypi'``.
        ...
        # ── 1. The author's declared page, or the module dir's OVERVIEW.md ──
        docs_dir = module_dir_path(pkg)
        declared = f"{docs_dir}{pkg.notes}" if (docs_dir and pkg.notes) else ""
        default = f"{docs_dir}OVERVIEW.md" if docs_dir else ""
        for candidate_path in (declared, default):
            if not candidate_path:
                continue
            local = Path(candidate_path)
            if local.is_file():
                return local.read_text()
            url = resolve_row_path(pkg, candidate_path, form="raw")
            if url:
                content = await asyncio.to_thread(_first_reachable, [url])
                if content:
                    return content
