...
class ShareFlow(StepFlow):
    ...
    def share_url(self) -> tuple[str | None, str | None, str | None]:
        ...
        One pair of URLs, not two. A project publishing to PyPI declares
        ``[tool.haywire.marketstall].distribute = "pypi"``, which makes the file
        at these URLs *be* the PyPI feed — so there is no separate deployed feed
        to advertise alongside them.
        """
