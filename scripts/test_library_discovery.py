...
def test_library_discovery():
    ...
    service = create_library_system_service(
        workspace_root=str(project_root), enable_file_watching=False, watch_settings=False
    )
...
def check_example_library():
    ...
    try:
        ...
        result = subprocess.run(["uv", "pip", "list", "--editable"], capture_output=True, text=True)
