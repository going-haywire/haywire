"""Tests for MarketplaceState (Plan E Phase 3 Task 19)."""
...
def test_on_enable_resolves_workspace_root(state_with_workspace) -> None:
    ...
    # for Task 19 just verify on_enable wired up.
    assert state._workspace_root == workspace


@pytest.mark.unit
def test_get_project_packages_returns_empty_when_no_cache(state_with_workspace) -> None:
    """Fresh project with no refresh yet → empty package list."""
...
@pytest.mark.unit
def test_get_project_packages_reads_existing_cache(state_with_workspace) -> None:
    state, workspace = state_with_workspace
    # Write a project marketplace with one package.
...
def test_get_global_returns_parsed(state_with_workspace, fake_home) -> None:
    """get_global parses ~/.haywire/marketplace.toml via marketplace_runtime."""
    state, _ = state_with_workspace
...
def test_get_global_returns_none_on_malformed(state_with_workspace, fake_home) -> None:
    """When ~/.haywire/marketplace.toml is malformed, get_global returns None
...
def test_refresh_invokes_runtime_refresh(state_with_workspace, fake_home, monkeypatch) -> None:
    """refresh() delegates to marketplace_runtime.refresh and caches the report."""
    state, workspace = state_with_workspace

    # Set up a marketstall subscription in the global.
    global_mp = fake_home / ".haywire" / "marketplace.toml"
    global_mp.parent.mkdir(parents=True, exist_ok=True)
    ...
    # Project marketplace was written with the resolved package.
    pkgs = state.get_project_packages()
