...
class TestLibrariesAndRegistries:
    ...
    def test_library_system_initialized(
        self,
    ...
    def test_multiple_registries_loaded(
        self,
        library_system: LibrarySystemService
    ):
    ...
    def test_node_factory_with_libraries(
        self,
        library_system: LibrarySystemService
    ):
        ...
        if available_nodes:
            ...
            try:
                node_instance = node_factory.create_node(
                    first_node_key,
                    'test_node_1',
                    graph
                )
                ...
                pytest.skip(
                    f"Could not create node {first_node_key}: {e}"
                )
