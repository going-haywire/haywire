...
def test_enrich_sets_graph_and_edge_id():
    exc = _exc().enrich(graph_id="/tmp/g.haywire", edge_id="n1[o]->n2[i]")
    assert exc.graph_id == "/tmp/g.haywire"
    assert exc.edge_id == "n1[o]->n2[i]"
...
def test_can_reveal_instance_requires_graph_plus_node_or_edge():
    ...
    # graph + edge
    assert _exc().enrich(graph_id="/tmp/g.haywire", edge_id="a[o]->b[i]").can_reveal_instance() is True
...
def test_to_dict_includes_locator_fields():
    exc = _exc().enrich(graph_id="/tmp/g.haywire", node_id="n1", edge_id="a[o]->b[i]")
    d = exc.to_dict()
    assert d["graph_id"] == "/tmp/g.haywire"
    assert d["node_id"] == "n1"
    assert d["edge_id"] == "a[o]->b[i]"
...
def test_edge_error_carries_edge_id_via_enrich():
    ...
    exc = _exc("bad edge").enrich(edge_id="n1[out]->n2[in]")
    assert exc.edge_id == "n1[out]->n2[in]"
