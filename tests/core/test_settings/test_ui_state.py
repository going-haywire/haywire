...
class TestSeed:
    def test_field_without_ui_state_starts_normal(self):
        bag = UiStateSettings()
        assert bag._ui_state("normal") is UiState.NORMAL

    def test_seeded_disabled_and_hidden(self):
        bag = UiStateSettings()
        assert bag._ui_state("starts_disabled") is UiState.DISABLED
        assert bag._ui_state("starts_hidden") is UiState.HIDDEN

    def test_storage_is_sparse(self):
        bag = UiStateSettings()
        # Only the two non-NORMAL seeds occupy the dict.
        assert len(bag._ui_states) == 2
...
class TestSetUiState:
    def test_set_and_reset_roundtrip(self):
        ...
        assert bag._ui_state("normal") is UiState.HIDDEN
        bag._set_ui_state("normal", UiState.DISABLED)
        assert bag._ui_state("normal") is UiState.DISABLED
        bag._set_ui_state("normal", UiState.NORMAL)
        assert bag._ui_state("normal") is UiState.NORMAL
        assert "normal" not in bag._ui_states  # NORMAL entries are dropped, not stored
    ...
    def test_set_ui_state_does_not_affect_value_or_writes(self):
        ...
        assert bag._ui_state("normal") is UiState.HIDDEN  # unaffected by the value write

    def test_seeded_field_can_be_reset_to_normal(self):
        bag = UiStateSettings()
        bag._set_ui_state("starts_disabled", UiState.NORMAL)
        assert bag._ui_state("starts_disabled") is UiState.NORMAL

    def test_ui_state_unknown_field_returns_normal(self):
        bag = UiStateSettings()
        assert bag._ui_state("nonexistent") is UiState.NORMAL

    def test_set_ui_state_unknown_field_warns_and_ignores(self, caplog):
        bag = UiStateSettings()
        with caplog.at_level(logging.WARNING):
            bag._set_ui_state("nonexistent", UiState.HIDDEN)
...
class TestSetUiStateAll:
    def test_bulk_covers_every_field(self):
        bag = UiStateSettings()
        bag._set_ui_state_all(UiState.DISABLED)
        for name in ("normal", "starts_disabled", "starts_hidden", "flag", "adv_a", "adv_b"):
            assert bag._ui_state(name) is UiState.DISABLED

    def test_bulk_normal_clears_seeded_states_too(self):
        bag = UiStateSettings()
        bag._set_ui_state_all(UiState.NORMAL)
        assert bag._ui_state("starts_disabled") is UiState.NORMAL
        assert bag._ui_state("starts_hidden") is UiState.NORMAL

    def test_bulk_category_filter_touches_only_that_category(self):
        bag = UiStateSettings()
        bag._set_ui_state_all(UiState.HIDDEN, category="advanced")
        assert bag._ui_state("adv_a") is UiState.HIDDEN
        assert bag._ui_state("adv_b") is UiState.HIDDEN
        assert bag._ui_state("normal") is UiState.NORMAL  # root category untouched

    def test_bulk_unknown_category_warns_and_ignores(self, caplog):
        bag = UiStateSettings()
        with caplog.at_level(logging.WARNING):
            bag._set_ui_state_all(UiState.HIDDEN, category="no_such_category")
        assert any("no_such_category" in rec.message for rec in caplog.records)
        assert len(bag._ui_states) == 2  # only the seeds

    def test_bulk_fires_one_transition_per_changed_field(self):
        bag = UiStateSettings()
        calls: list[tuple[str, UiState]] = []
        bag._subscribe_ui_state(lambda name, state: calls.append((name, state)))
        bag._set_ui_state_all(UiState.DISABLED)
        ...
        calls.clear()
        bag._set_ui_state_all(UiState.DISABLED)  # idempotent — fully silent second time
        assert calls == []
...
class TestUiStateChannel:
    def test_listener_fires_on_transition_only(self):
        ...
        bag._subscribe_ui_state(lambda name, state: calls.append((name, state)))
        bag._set_ui_state("normal", UiState.HIDDEN)
        bag._set_ui_state("normal", UiState.HIDDEN)  # idempotent — must NOT fire again
        bag._set_ui_state("normal", UiState.NORMAL)
        bag._set_ui_state("normal", UiState.NORMAL)  # idempotent — must NOT fire again
    ...
    def test_disabled_to_hidden_is_a_transition(self):
        ...
        calls: list[tuple[str, UiState]] = []
        bag._subscribe_ui_state(lambda name, state: calls.append((name, state)))
        bag._set_ui_state("starts_disabled", UiState.HIDDEN)
    ...
    def test_redundant_set_on_seeded_state_does_not_fire(self):
        ...
        calls: list[tuple[str, UiState]] = []
        bag._subscribe_ui_state(lambda name, state: calls.append((name, state)))
        bag._set_ui_state("starts_disabled", UiState.DISABLED)  # already DISABLED via seed
        assert calls == []
    ...
    def test_unsubscribe_stops_delivery(self):
        ...
        bag._subscribe_ui_state(listener)
        bag._set_ui_state("normal", UiState.DISABLED)
        bag._unsubscribe_ui_state(listener)
        bag._set_ui_state("normal", UiState.NORMAL)
...
class TestEffectiveUiState:
    def test_ungated_field_is_normal(self):
        bag = GatedSettings()
        assert bag._effective_ui_state("enable_color") is UiState.NORMAL

    def test_enabled_when_contributes_at_most_disabled(self):
        bag = GatedSettings()
        bag.enable_color = False
        assert bag._effective_ui_state("exposure") is UiState.DISABLED

    def test_visible_when_contributes_hidden(self):
        bag = GatedSettings()
        bag.enable_color = False
        assert bag._effective_ui_state("manual_focus") is UiState.HIDDEN

    def test_satisfied_gates_leave_normal(self):
        bag = GatedSettings()
        assert bag.enable_color is True  # default
        assert bag._effective_ui_state("exposure") is UiState.NORMAL
        assert bag._effective_ui_state("manual_focus") is UiState.NORMAL

    def test_severity_max_manual_hidden_beats_declarative_disabled(self):
        bag = GatedSettings()
        bag.enable_color = False  # enabled_when → DISABLED
        bag._set_ui_state("exposure", UiState.HIDDEN)  # imperative → HIDDEN
        assert bag._effective_ui_state("exposure") is UiState.HIDDEN
    ...
    def test_manual_state_composes_when_gates_are_satisfied(self):
        bag = GatedSettings()
        bag._set_ui_state("exposure", UiState.DISABLED)
        assert bag._effective_ui_state("exposure") is UiState.DISABLED

    def test_unknown_controller_is_skipped_silently(self):
        bag = GatedSettings()
        assert bag._effective_ui_state("typo_gated") is UiState.NORMAL

    def test_unknown_name_returns_normal(self):
        bag = GatedSettings()
        assert bag._effective_ui_state("nonexistent") is UiState.NORMAL

    def test_effective_state_never_touches_cells(self):
        bag = GatedSettings()
        events: list[str] = []
        bag._subscribe(lambda name, value, old: events.append(name))
        bag._effective_ui_state("exposure")
        bag._effective_ui_state("manual_focus")
        assert events == []
