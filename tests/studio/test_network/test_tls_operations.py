...
from haywire_studio.security.document import NetworkPolicy, SecurityDocument, save_document
...
def env(tmp_path, monkeypatch):
    """A certificate directory and a document file, isolated from ~/.haywire."""
...
def _write_settings(env, **network):
    save_document(SecurityDocument(network=NetworkPolicy(**network)), env.settings)


def _write_raw_network(env, **network):
    """Write a network block straight to disk, skipping ``validate``.

    Some fixtures deliberately build a document ``save_document`` would refuse
    (exposed with no TLS/auth, a half-configured TLS pair) — exactly the
    hand-edited state ``status`` must still classify without raising.
    """
    env.settings.write_text(
        json.dumps({"version": 1, "network": network}),
        encoding="utf-8",
    )
...
def test_setup_creates_both_files_and_configures_them(env):
    result = setup(directory=env.directory, path=env.settings)
    assert result.cert_path.exists()
    assert result.key_path.exists()
    data = json.loads(env.settings.read_text(encoding="utf-8"))
    assert data["network"]["tls_certfile"] == str(result.cert_path)
    assert data["network"]["tls_keyfile"] == str(result.key_path)
...
def test_setup_merges_extra_names(env):
    result = setup(["studio.example.com"], directory=env.directory, path=env.settings)
...
def test_setup_twice_is_refused(env):
    setup(directory=env.directory, path=env.settings)
...
def test_setup_adopts_an_orphaned_certificate_without_regenerating(env):
    ...
    first = setup(directory=env.directory, path=env.settings)
    before_cert, before_key = _digest(first.cert_path), _digest(first.key_path)

    env.settings.write_text(json.dumps({"version": 1, "network": {}}), encoding="utf-8")
    ...
    data = json.loads(env.settings.read_text(encoding="utf-8"))
    assert data["network"]["tls_certfile"] == str(first.cert_path)
...
def test_update_without_a_certificate_is_refused(env):
    with pytest.raises(CertError, match="ssl setup"):
        update(directory=env.directory, path=env.settings)


def test_update_reuses_the_private_key(env):
    created = setup(directory=env.directory, path=env.settings)
    key_before = _digest(created.key_path)
    update(add=["10.9.9.9"], directory=env.directory, path=env.settings)
    assert _digest(created.key_path) == key_before


def test_update_add_preserves_existing_names(env):
    setup(["manual.example.com"], directory=env.directory, path=env.settings)
    result = update(add=["10.9.9.9"], directory=env.directory, path=env.settings)
...
def test_update_refresh_unions_local_names_keeping_manual_ones(env, monkeypatch):
    ...
    result = update(refresh=True, directory=env.directory, path=env.settings)
...
def test_update_can_remove_a_name(env):
    setup(["gone.example.com"], directory=env.directory, path=env.settings)
    result = update(remove=["gone.example.com"], directory=env.directory, path=env.settings)
...
def test_update_refuses_to_remove_loopback(env):
    ...
    with pytest.raises(CertError, match="localhost"):
        update(remove=["localhost"], directory=env.directory, path=env.settings)
...
def test_status_off_loopback_when_not_exposed(env):
    assert status(directory=env.directory, path=env.settings).state is TlsState.OFF_LOOPBACK


def test_status_off_exposed_when_exposed(env):
    _write_raw_network(env, exposed=True)
    assert status(directory=env.directory, path=env.settings).state is TlsState.OFF_EXPOSED


def test_status_ok_after_setup(env):
    setup(directory=env.directory, path=env.settings)
    result = status(directory=env.directory, path=env.settings)
...
def test_status_file_missing_when_the_certificate_was_deleted(env):
    ...
    assert "tls_certfile" in result.detail


def test_status_half_configured(env):
    _write_raw_network(env, tls_certfile="/somewhere/studio.crt")
    result = status(directory=env.directory, path=env.settings)
    assert result.state is TlsState.HALF_CONFIGURED
    assert "tls_keyfile" in result.detail


def test_status_orphaned_when_settings_were_cleared(env):
    setup(directory=env.directory, path=env.settings)
    env.settings.write_text(json.dumps({"version": 1, "network": {}}), encoding="utf-8")
    assert status(directory=env.directory, path=env.settings).state is TlsState.ORPHANED


def test_status_key_mismatch(env):
    setup(directory=env.directory, path=env.settings)
    certs.write_key(certs.generate_key(), env.directory)
    assert status(directory=env.directory, path=env.settings).state is TlsState.KEY_MISMATCH


def test_status_unreadable_certificate(env):
    created = setup(directory=env.directory, path=env.settings)
    created.cert_path.write_text("not a certificate", encoding="utf-8")
    assert status(directory=env.directory, path=env.settings).state is TlsState.UNREADABLE
...
def test_status_expiring(env, monkeypatch):
    ...
    assert status(directory=env.directory, path=env.settings).state is TlsState.EXPIRING
