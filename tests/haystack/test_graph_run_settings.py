...
def test_to_dict_emits_non_default():
    ...
    s = GraphRunSettings()
    s.autorestart = True
    assert s._to_dict() == {"values": {"autorestart": True}, "promoted": {}}
...
def test_from_dict_restores_value():
    ...
    s = GraphRunSettings()
    s._from_dict({"values": {"autorestart": True}, "promoted": {}})
    assert s.autorestart is True
...
def test_from_dict_ignores_unknown_keys():
    ...
    s = GraphRunSettings()
    s._from_dict({"values": {"autorestart": True, "future_flag": "x"}, "promoted": {}})
    assert s.autorestart is True
