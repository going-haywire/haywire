...
**Created by:** `haywire init --dev`
When the `--dev` flag is passed, `init_project()` calls `_generate_dev_marketplace()`
which writes a `marketplace.toml` listing all libraries in the dev repo with
...
Or use the Library Manager UI: paste the absolute path into the install field of an
Available entry, or add a `source = "local"` entry in the project marketplace.
