...
Promoting a setting used to mean "create a separate DATA inlet and bridge reads back to it" — a **two-value** design where the port has one value, the setting resolves another, and a bridge forwards reads. This ADR records replacing it with **reference-sharing**: a promoted port borrows the setting's [P4 cell](0013-settings-single-cell.md) *by reference* (via a new `DataPort.bind_field`), so a setting and its promoted port are **one cell, two views** — there is no second value, no `_promoted_port_id`, and no read-tier bridge. It is plan **P5**, the final plan of the settings↔DataField unification arc (canonical-key → tier-collapse → JSON → single-cell → **promotion-as-direction**), and it completes the arc.
...
- **A promoted CONFIG keeps its Properties-panel widget (2026-09-08).** Supersedes the sentence above that had a promoted CONFIG row render "exactly like a promoted INLET". The inlet rule exists because the *graph* owns an inlet's value — an edge can overwrite whatever the panel writes. A CONFIG port has no pin and no edge, so nothing external can ever own its value: the setting remains the source of truth, which is the OUTLET case, not the INLET one. Hiding the panel widget therefore removed the field's only Properties-panel write path for no protective reason. The row now keeps its editable widget (hint `"promoted to config"`, `data-promoted-direction="config"`) and its Reset stays actionable, exactly as for an outlet; `is_promoted_input` in `render_utils._render_reactive_field_row` again means only "inlet-driven". Promoting to config *adds* the port's own live widget wherever a `PortType.CONFIG` port renders (Ports Panel / node card) rather than moving the panel's. Unchanged: `Promotable.INPUT = INLET | CONFIG` and the locally-set mark at promote time — that is value *resolution* (the setting's read must return the shared cell instead of falling through mirror resolution), not a UI concern.
