...
| **Promotion** | Assigning a **direction** to a value-bearing setting and surfacing it as a DATA port. A promoted port and its setting are *one cell, two views* — the port borrows the setting's `DataField` cell by reference and its id IS the setting's `storage_key`. The port stays the promotion signal (serialized in the ports block); demote just removes it. Two verbs (`promote(direction)` / `demote`), two directions (inlet / outlet). See [ADR 0015](../adr/0015-promotion-storage-key-id.md) (mechanism), [ADR 0014](../adr/0014-promotion-as-direction.md) (direction model) | Exposing, hoisting, wiring a setting |
