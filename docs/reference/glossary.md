...
| **promoted inlet / promoted outlet** | The two promotion directions. A promoted **inlet** lets an edge drive the setting (the panel widget is gone as soon as the promotion exists, linked or not); a promoted **outlet** lets the setting drive downstream consumers. A `watch()` field is outlet-only (read-only ⇒ no write path in); a `watch()` field renders a read-only live-value row so its outlet promotion has a surface | — |
