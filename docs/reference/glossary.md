...
| **Marketstall** | A TOML file hosted by a library author that lists their published haybale packages; consumed as a remote feed by haywire projects. Produced by `haywire share`. Lives at `marketstall.toml` in the repo root. Any author can host their own; the haywire monorepo publishes its official one to GitHub Pages. | marketplace snippet, package feed |
| **Marketplace** | The aggregated catalog of packages visible in the Library Manager's "Available" section; built by merging the project's local `.haywire/marketplace.toml` with all remote **Marketstall** feeds configured by the user. Multiple remote feeds are supported. | package list, library catalog |
| **haybale-marketplace** | The planned haybale package that will own the Library Manager UI and all marketplace/marketstall logic; currently embedded in `haybale-studio`. Exact scope and split point TBD. | Library Manager (use haybale-marketplace once the split lands) |
