...
| **Drift gate** | The `check_drift()` step in `SharePipeline` (`haywire_studio.share.pipeline`), run early in every `haywire share` mode. Three checks per library: `pyproject_missing` (imports not declared in `pyproject.toml`), `decorator_missing` (imports not declared in `@library(dependencies=[...])`), `pyproject_version_lag` (declared haybale floors lagging the installed version). Interactive mode surfaces it as a step to resolve; `--yes` refuses to run while any drift is unresolved. `haywire deps check` (`haywire_studio.deps_cli`) runs the same per-library `detect_share_drift()` check CI-side, decoupled from `SharePipeline` — no git, no preconditions, no versioning; read-only, exits 1 on drift and 0 otherwise. Spec §12. | — |
