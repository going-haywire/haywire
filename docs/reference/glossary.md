...
| **Promotion** | Assigning a **direction** to a value-bearing setting and surfacing it as a DATA port. A promoted port and its setting are *one cell, two views* — the port borrows the setting's `DataField` cell by reference and its id IS the setting's `storage_key`. Promotion state lives in the owning settings bag (`Settings._promoted_keys`, `storage_key → direction`) — the single source of truth. The promoted port is **regenerated from settings on load, never serialized** in the ports block (ADR 0019); `promote_setting` records the promotion and `demote_setting` clears it (mirror operations). Two verbs (`promote(direction)` / `demote`), two directions (inlet / outlet). Which directions a field may ever be promoted to is gated by `setting(promotable=Promotable.NONE\|INLET\|OUTLET\|ALL)` (default `ALL`), via the single `eligible_promotion_directions()` oracle shared by `promote_setting` (raises for an ineligible direction) and the promote menu (hides ineligible entries). **Direction- and link-aware editing surface (decision 7bA):** a setting promoted to an **inlet** renders read-only in the settings panel — a live label, not an editable widget — because the graph now owns the value; the hint reads "↳ promoted to inlet" while unlinked or "↳ driven by inlet" once an edge is connected. A setting promoted to an **outlet** keeps its editable widget (the setting is still the source of truth), hinted "↳ promoted to outlet". The row carries `data-promoted-direction="inlet"\|"outlet"`. See [ADR 0014](../adr/0014-promotion-as-direction.md) (direction model + one-cell-two-views) and [ADR 0019](../adr/0019-settings-owned-promotion.md) (settings-owned state + non-serialized regeneration, superseding 0014's amendment) | Exposing, hoisting, wiring a setting |
...
| **Flyout menu** | A nested context-menu submenu that opens **on hover** of its anchor item and cascades to the right (`anchor="top end" self="top start"`), raised above the context-menu popup (`z-index 7100 > 7001`). Only one open path from the root exists at a time: opening one flyout closes its **siblings** (same-level group) and their open descendants, via an explicit `_close_flyout` cascade — closing on click-away is left to Quasar's `auto-close`. The fragile hover-open / sibling-close mechanics are shared by the **add-node menu** and the **Promote-Setting menu** through a common flyout primitive. See [.insights/feedback_nicegui_nested_menu_flyouts.md] | submenu, cascade menu, nested QMenu |
| **Promote-Setting menu** | The node right-click submenu listing a node's promotable `setting()` fields as a hierarchical **Flyout menu** (`bag ▸ field ▸ direction`), mirroring the add-node menu's shape. A single-direction (read-only `watch()`) field collapses to a labeled leaf (`field → outlet`); a two-direction field stays a `field ▸ [inlet\|outlet]` flyout. The bag level is always rendered. Replaces the earlier flat `accessor.field → verb` button list | promote submenu, promotion menu |
