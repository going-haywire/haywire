...
Once the marketstall-distribution spec lands ([`internals/specs/marketstall-distribution.md`](../../internals/speculatives/archive/marketstall-distribution.md)), the word **haybale** carries three closely-related but distinct meanings. They cluster around the same concept (a haywire library and its distribution metadata) but project it onto different surfaces.
...
| **haybale-marketplace** | The planned haybale package that will own the Library Manager UI and all marketplace/marketstall logic; currently embedded in `haybale-studio`. The directory `~/.haywire/db/haybale-marketplace/` is already used by the marketstall runtime as a forward reference to this planned carve-out. Exact scope and split point TBD. | Library Manager (use haybale-marketplace once the split lands) |
...
The runtime that backs the Library Browser's Refresh / Add Source / Edit File flow. Spec: [`internals/specs/marketstall-distribution.md`](../../internals/speculatives/archive/marketstall-distribution.md). Code: `haywire.core.marketstall`.
