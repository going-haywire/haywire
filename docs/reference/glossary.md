...
| **Active axis** | The single *primary* element of a selection — `EditState.active_node` / `active_edge` (one wrapper or `None`). The **inspector subject**: the element whose props / ports / settings / errors the Properties panels render. Derived from the Selection axis as its primary member (today `next(iter(selected_nodes))`); a multi-selection still has exactly one active element. Distinct from the **Selection axis**. | active node (overloaded with CONTROL node — see Node Behaviors), focused node |
