...
| **Promotion** | Assigning a **direction** to a value-bearing setting and surfacing it as a DATA port. A promoted port and its setting are *one cell, two views* — the port borrows the setting's `DataField` cell by reference and its id IS the setting's `storage_key`. The port stays the promotion signal (serialized in the ports block); demote just removes it. Two verbs (`promote(direction)` / `demote`), two directions (inlet / outlet). **Direction- and link-aware editing surface (decision 7bA):** a setting promoted to an **inlet** renders read-only in the settings panel — a live label, not an editable widget — because the graph now owns the value; the hint reads "↳ promoted to inlet" while unlinked or "↳ driven by inlet" once an edge is connected. A setting promoted to an **outlet** keeps its editable widget (the setting is still the source of truth), hinted "↳ promoted to outlet". The row carries `data-promoted-direction="inlet"|"outlet"`. See [ADR 0015](../adr/0015-promotion-storage-key-id.md) (mechanism), [ADR 0014](../adr/0014-promotion-as-direction.md) (direction model) | Exposing, hoisting, wiring a setting |
