...
| **Drift gate** | The `check_drift()` step in `SharePipeline` (`haywire_studio.packaging.share.pipeline`), run early in every `haywire share` mode. Three checks per library: `pyproject_missing` (imports not declared in `pyproject.toml`), `decorator_missing` (imports not declared in `@library(dependencies=[...])`), `pyproject_version_lag` (declared haybale floors lagging the installed version). Interactive mode surfaces it as a step to resolve; `--yes` refuses to run while any drift is unresolved. `haywire deps check` (`haywire_studio.packaging.deps`) runs the same per-library `detect_share_drift()` check CI-side, decoupled from `SharePipeline` — no git, no preconditions, no versioning; read-only, exits 1 on drift and 0 otherwise. Spec §12. | — |
...
| **DepDrift** | Dataclass from `haywire_studio.packaging.share.drift.model` recording one library's missing entries. Fields: `lib_dir`, `pyproject_missing`, `decorator_missing`, `pyproject_version_lag`, `unresolved`. `has_drift` is True iff either missing-list is non-empty. The gate flags missing entries only; extra (declared but unused) deps are not flagged. Wrapped per-library into the pipeline's `DriftReport` (see **Drift gate** above). | — |
| **ShareError** | Base exception for every `haywire share` failure (`haywire_studio.packaging.share.pipeline.errors`), including refusing `--yes` with unresolved drift. Subclasses cover each pipeline step (`PreconditionsError`, `ManifestError`, `VersionError`, `TagCollisionError`, `DocsGenerationError`, `MarketstallError`, `CommitError`, `PushError`). Caught once at the CLI boundary in `run_share_cli` and per-step in the wizard; never lets a raw traceback reach the user. | DriftError (old, single-purpose exception; superseded — do not revive) |
...
> **Domain expert:** "Every `haywire share` mode runs the **Drift gate** early. `--yes` refuses to proceed if any of your barn libraries are missing manifest entries the source actually imports; interactive mode stops on the same step and lets you resolve it (Detect → Union) before continuing. In CI, `haywire deps check` runs the same gate read-only, independent of `SharePipeline` — it exits 1 on drift and 0 otherwise, and never writes."
