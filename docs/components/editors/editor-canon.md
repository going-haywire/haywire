...
When a `draw_tab` override reads state that changes *outside* a normal redraw cycle — a live badge count, a status tint — call **`self.wrapper.refresh_tab_bar()`** to repaint the bar interior now (e.g. from a `@react_on` handler). It is the dirtiness-free counterpart to `set_dirty(refresh=True)` (which is *only* for the unsaved-content flag and its `•` glyph): both share one repaint under the hood, but `refresh_tab_bar()` changes no wrapper state. No-op for detached wrappers.
