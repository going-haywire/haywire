...
**Boundaries.** Framework mechanics — registry events, container subscription, hot-reload coordination, lifecycle ordering — live in [architecture/session-and-state](../../architecture/session-and-state/session-and-state-arch.md). The `LibrarySettings` system that AppState compositions sometimes wrap lives in [components/settings](../settings/setting-canon.md). The `Library` class that registers state folders lives in [haybale-canon](../../haybale/haybale-canon.md).
