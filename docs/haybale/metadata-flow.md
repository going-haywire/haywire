...
| `pyproject.toml`   | split           | author (`version`, `dependencies`) + publish (the rest) |              | [fields](../reference/files/pyproject-toml.md)   |
| `marketstall.toml` | `haywire share` | publish, fully autogenerated                            |              | [fields](../reference/files/marketstall-toml.md) |
...
`pyproject.toml` keeps `version` and `dependencies` as its sole canonical field plus the
...
| Takes      | `id`, `version`, `label`, `linked_libraries`, `on_reload` | Whatever it is about to display   |
...
that cannot each afford a file read; `version` is reported by ~10 call sites
(the marketplace's update-available check, the docs generator, farmhand's
catalog) that likewise read it off the identity rather than the file.
