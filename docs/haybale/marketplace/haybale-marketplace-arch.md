...
|---|---|
| **REQUIRED** | Installed + enabled + some other installed haywire library declares it in its `@library(linked_libraries=[...])`. The signal comes from `LibraryManager.get_installed_dependents(lib_id)` — the same source the Overview Editor's Disable / Uninstall gating uses. |
...
2. It computes two diffs: the current `@library(linked_libraries=[...])` value vs detected, and the current `[project] dependencies` in the library's `pyproject.toml` vs detected.
