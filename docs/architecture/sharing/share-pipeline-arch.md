...
- `apply_decorator_registrations({lib: [name, …]})` — adds imported haywire libraries to `@library(linked_libraries=[...])`. **Applied without asking**, at the framework step so it lands exactly once. There is nothing to decide: `detect_deps` emits a name here only when the source imports it *and* it resolves to an installed registered library, so every entry is provably true, carries no version specifier, and narrows nothing for consumers. It is *reported* on the Findings and Confirm screens rather than done silently, because it edits a hand-authored `__init__.py` rather than a manifest.
