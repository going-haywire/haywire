...
class DataPortIdentity(BaseIdentity):
    ...
    def as_inlet(self, id: str, **kwargs) -> 'PortInlet':
        """
        Create a PortInlet from this identity.
        
        Args:
            id: Port identifier within the node (e.g., 'input_value')
            **kwargs: Override any identity attributes or add port-specific fields
                     (flow_type, callback, is_pooled, etc.)
        
        Returns:
            PortInlet instance with all identity attributes
        """
        ...
        # Remove default from kwargs, it needs be dealt with separately
        kwargs.pop('default', None)

        # Store creation recipe for serialization (if from registered type)
        if self.registry_key and not self.registry_key.startswith('default:'):
            inlet._creation_recipe = {
                'registry_key': self.registry_key,
                'method': 'as_inlet',
                'kwargs': kwargs  # Store the modified kwargs dict
            }
        
        return inlet
    
    def as_outlet(self, id: str, **kwargs) -> 'PortOutlet':
        """
        Create a PortOutlet from this identity.
        ...
        Returns:
            PortOutlet instance with all identity attributes
        """
