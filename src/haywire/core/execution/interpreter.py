...
class Interpreter:
    ...
    def _register_flow(self, flow: 'Flow'):
        ...
        logger.debug(
            f"Flow {flow.flow_id} registered for event '{subscription_key}'"
        )
        ...
        if isinstance(flow.event_subscription, CallbackEvent):
            self.callback_manager.register_callback_listener(
                flow.event_subscription.event_name,
                flow
            )
    ...
    def _dispatch_event(
        ...
        logger.debug(
            f"Dispatching event '{subscription_key}' to {len(flows)} flows"
        )
        
        # Create trigger
        trigger = Trigger(
            source_key=subscription_key,
            payload=payload,
            timestamp=time.time()
        )
        ...
        for flow in flows:
            if flow.scheduler:
                ...
                logger.warning(
                    f"Flow {flow.flow_id} has no scheduler, cannot trigger"
                )
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Interpreter(graph={graph_id}, "
            f"subscriptions={len(self.event_subscriptions)})"
        )
