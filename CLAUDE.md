...
- [project_rejig_orphans_edge_port_refs.md](.insights/project_rejig_orphans_edge_port_refs.md) — `rejig` replaces port OBJECTS and an `EdgeWrapper` holds its endpoints by reference, so a replaced port leaves edges pointing at an orphan: `detach()` unlinks that one and the live port stays linked for good, silently. A node rejigging from a `hb_*` callback is saved by the next validation batch re-resolving endpoints by id; one rejigging INSIDE a validation callback (`GraphNode.reconcile_interface`) is not. Diagnose with `edge._inlet_port is node.ports[pin_id]`, not by reading the render path.
